...
class BlurActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel.outputStatus.observe(this, Observer {
            listWorkStatus->
                if (listWorkStatus == null || listWorkStatus.isEmpty()) {
                    return@Observer
                }

            listWorkStatus[0].let { workStatus ->
                if (workStatus.state.isFinished) {
                    showWorkFinished()
                    val outputUri = workStatus.outputData.getString(KEY_IMAGE_URI)
    ...
    private fun setOnClickListeners() {
        ...
        cancelButton.setOnClickListener { viewModel.cancleWork() }
        outputButton.setOnClickListener {
            viewModel.outputUri?.let { outputFileUri ->
                val intent = Intent(Intent.ACTION_VIEW, outputFileUri)
